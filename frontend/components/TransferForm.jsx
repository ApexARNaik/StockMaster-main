...
const TransferForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (!formData.sourceWarehouseId || !formData.destWarehouseId || formData.items.length === 0) {
      ...
      setMessage(error.response?.data?.error || 'Error creating transfer');
    }
  };
  ...
  const handleStatusChange = async (docId, newStatus) => {
    try {
      ...
      setMessage('Transfer status updated!');
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">↔️ Transfers</h1>
        <button
        ...
        <div className="app-card">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="app-label">Source Warehouse *</label>
                <select
            ...
            {filteredDocuments.length === 0 ? (
              ...
              filteredDocuments.map((doc) => (
                <tr key={doc.id} className="border-b border-dark-border hover:bg-dark-surface">
                  ...
                  <td className="p-3">{doc.sourceWarehouse?.name || 'N/A'}</td>
